...
from app.services.copilot.intent_schemas import TopicUpdateResult
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            result = await llm.generate_structured(
                ...
                schema=TopicUpdateResult,
                temperature=0.1,
                capability="topic_tracking",
            )

            if result.action == "new_topic" and existing is not None:
                existing.status = "completed"
                await db.flush()
                existing = None

            if existing is None:
                existing = AITopic(session_id=session_id, title=result.title[:255])
                db.add(existing)

            existing.title = (result.title or existing.title)[:255]
            existing.summary = result.summary or existing.summary
            existing.open_items = result.open_items or existing.open_items or []
            await db.commit()
            logger.info(
                "Topic updated: session=%s action=%s title=%r summary=%r open_items=%s",
                session_id, result.action, existing.title, existing.summary, existing.open_items,
            )
    except Exception:
